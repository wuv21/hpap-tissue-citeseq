...
names(attributes(so))
names(attributes(so@assays$RNA))
sum(apply(GetAssayData(so, slot="data"), 1, function(x) length(which(x==0))))
dim(GetAssayData(so, slot="data"))
d1=as.numerix(dim(GetAssayData(so, slot="data"))[1])
d2=as.numeric(dim(GetAssayData(so, slot="data"))[2])
650086*32581
d1*d2
Matrix::nnzero(GetAssayData(so, slot="data"))/(d1*d2) #data
Matrix::nnzero(GetAssayData(so, slot="scale.data"))/(d1*d2) #scale data
Matrix::nnzero(GetAssayData(so, slot="counts"))/(d1*d2) #counts
...
so[["combined_clust_grp"]] = ifelse(so[["manualAnnot"]][,1] %in% bcell_clusters, "All B Cells combined", NA)
so[["combined_clust_grp"]] = ifelse(so[["manualAnnot"]][,1] %in% nk_clusters, "All NK Cells combined", so[["combined_clust_grp"]][,1])
unique(so[["combined_clust_grp"]])
