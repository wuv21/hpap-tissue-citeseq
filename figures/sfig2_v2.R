...
dfCD4Heatmap2 <- dfLineageFilter %>%
  filter(Staining_Flow_Researcher != "A") %>% #differences in flow researcher was significant for many pops in CD4, this researcher had to be filtered out to avoid too many NA values
  filter(Tissue != "Spleen") %>%
  ...
  dplyr::select(1:LN_type, metric, z_score) %>%
  pivot_wider(id_cols = `HPAP Donor`:`LN_type`, names_from = metric, values_from = z_score) %>%
  mutate(Sample_ID = paste("S", as.character(row_number()), sep = "")) %>%
  relocate(Sample_ID, .before = `HPAP Donor`) %>%
  as.data.frame()

rownames(dfCD4Heatmap2) <- dfCD4Heatmap2$Sample_ID

dfCD4HeatmapMetadata2 <- dfCD4Heatmap2 %>%
  dplyr::select(Tissue) %>%
  dplyr::mutate(Tissue = gsub("_", " ", Tissue))

dfCD4Heatmap2 <- dplyr::select(dfCD4Heatmap2, "Tcm":ncol(dfCD4Heatmap2))
dfCD4Heatmap2 <- t(dfCD4Heatmap2)

# Heatmap
col_fun_CD4HM2 <- colorRamp2(c(-2, 0, 2), c("blue", "white", "red")) #set maxima at 5th and 95th percentiles
CD4Ann2 <- HeatmapAnnotation(
  df = dfCD4HeatmapMetadata2,
  which = "col",
  col = LinTissueColorsHeatmap,
  simple_anno_size = unit(0.1, "in"),
  # height = unit(0.05, "in"),
  annotation_name_gp = gpar(fontsize = BASEPTFONTSIZE - 2),
  annotation_legend_param = list(
    title = "Tissue",
    at = LinTissueOrder2,
    labels = LinTissueOrder2,
    ...
    title_gp = gpar(fontsize = BASEPTFONTSIZE - 2)
  ))
...
CD4Heatmap2 <- Heatmap(
  ...
  name = "Z-score",
  col = col_fun_CD4HM2,
  row_title_gp = gpar(fontsize = BASEPTFONTSIZE - 2),
  column_title_gp = gpar(fontsize = BASEPTFONTSIZE - 2),
  row_title_rot = 0,
  row_names_gp = gpar(fontsize = 4),
  row_dend_width = unit(0.1, "in"),
  column_dend_height = unit(0.2, "in"),
  show_column_names = FALSE,
  top_annotation = CD4Ann2,
